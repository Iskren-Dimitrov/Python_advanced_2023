...
guest_reservation = [input() for reservation_codes in range(int(input()))]
reservation_numbers = []

command = input()
while command != "END":
    reservation_numbers.append(command)
    command = input()

not_arrived_guests = set(guest_reservation).difference(reservation_numbers)
print(len(not_arrived_guests))

for guests in sorted(not_arrived_guests):
    print(guests)
